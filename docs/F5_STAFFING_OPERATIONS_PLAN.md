...
- [x] Billing: `Invoice` generada con líneas reales, balance calculado correctamente tras registrar al menos un pago de prueba — **implementado y verificado en F5.8** (ver §23). Opción B (modelo `Payment`) aprobada explícitamente por el PO tras someterse a decisión — no se asumió unilateralmente.
...
- [x] `pnpm typecheck`/`lint`/`test` limpios en todo el monorepo — verificado tras el cierre de F5.8 (320/320 tests, solo 2 warnings preexistentes no relacionados).
- [x] F0–F4.9 intactos — ningún test existente se modifica ni se rompe.
- [x] Verificación en navegador real (Playwright) del ciclo completo: candidato → worker → asignación → timesheet aprobado → payroll run aprobado → invoice con pago registrado — **verificado en F5.8** (ver §23.4), desktop y mobile, cero errores de consola.
- [x] Cero cambios de schema aplicados sin tu aprobación explícita previa de cada uno (§4.2, §9.2, §10.1) — cada migración de F5 se presenta y se aprueba antes de correrla, mismo protocolo que F4.9 ya siguió. El modelo `Payment` (§10.1) se sometió a decisión explícita y se aprobó Opción B antes de tocar schema.

**Con F5.8 cerrado, F5 — Staffing Operations queda completamente implementado (F5.1–F5.8). Matching por IA y Dashboards por rol (arriba, sin marcar) quedan fuera del alcance mandatado para F5 — no fueron parte de la orden de trabajo de esta fase.**
...
## 23. F5.8 — Billing (Invoice + Payment) — Resultado real (implementado, verificado, cerrado)

**Estado: completo. Con esto, F5 (F5.1–F5.8) queda completamente cerrado.**

### 23.1 Decisión bloqueante resuelta (aprobación explícita del PO)

§10.1 dejó explícitamente sin resolver el modelo de pago, sometiendo dos opciones a aprobación antes de tocar schema. Se presentó la decisión al PO (Opción A: campos `paidAmount`/`paidAt` planos en `Invoice`, vs. Opción B: modelo `Payment` nuevo con historial completo). **El PO aprobó la Opción B**, recomendada por este documento. `balance` se deriva siempre (`total - sum(Payment.amount)`), nunca una columna propia — mismo principio aplicado en todo el proyecto desde F1.

### 23.2 Decisiones tomadas con evidencia (no bloqueantes, documentadas para auditoría)

- **`PayrollItem.invoiced` (columna nueva):** el schema no tenía forma de marcar qué `billAmount` ya se facturó — sin esto, generar un segundo Invoice para el mismo período habría duplicado el cobro. Se agregó como flag booleano (mismo patrón que `TimeEntryStatus.LOCKED`: un estado de "ya consumido", no una relación bidireccional nueva). Migración puramente aditiva.
- **`invoices.send` como permiso especial sujeto a MFA:** ya estaba documentado como gap explícito desde F4.9 §6 ("el PO pidió invoices.send, no existe el módulo todavía — agregar cuando exista"). Se agregó ahora, exactamente como se dejó anticipado, sin inventar nada nuevo.
- **PAID y OVERDUE son siempre derivados, nunca alcanzables por transición manual:** PAID se deriva de que un Payment salde el balance a 0 (mismo criterio que JobOrder.status/Worker.status/Worker.complianceStatus en F5.4/F5.5); OVERDUE se deriva de un sweep periódico (dueDate vencida), mismo patrón que `runComplianceAlertSweepForTenant` de F5.5 — nuevo scheduler propio (`billing/scheduler.ts`, 60 min).
- **Solo se factura desde un PayrollRun `APPROVED` o posterior** (nunca `DRAFT`/`PENDING_APPROVAL`) — decisión de implementación razonable, no bloqueante: los costos recién se consideran facturables una vez aprobados internamente.
- **`dueDate` net-30 desde la creación**, hardcodeado como valor provisional — mismo criterio que `OT_MULTIPLIER` en F5.7, documentado, no oculto, hasta que exista un campo de términos de pago por Company.
- **Una línea de Invoice por Assignment/worker**, agregando horas + `billAmount` del `PayrollItem` — la opción que §10.2 dejó a criterio de implementación ("a decidir con el PO, probablemente una línea por worker").

### 23.3 Alcance implementado

- Schema: modelo `Payment` nuevo (`tenantId`, `invoiceId`, `amount`, `paidAt`, `method` texto libre, `reference`), relación `Invoice.payments`; columna `PayrollItem.invoiced`. Ambas migraciones puramente aditivas, aplicadas y verificadas (`migrate status`/`validate`/`generate`). `Payment` agregado a `STRICT_TENANT_MODELS`.
- Permisos: recurso `invoices` (view/create/update/delete) + especial `invoices.send` (sujeto a MFA). Accounting recibe el CRUD completo + send; Manager recibe view (mismo patrón de supervisión cross-cutting del resto de roles).
- Contratos compartidos: `invoiceStatusSchema` + `INVOICE_STATUS_TRANSITIONS`, `createInvoiceInputSchema`, `createPaymentInputSchema`, `invoiceListItemSchema`/`invoiceDetailSchema` con `paidTotal`/`balance` derivados.
- Backend (`billing/service.ts` + `router.ts`): `GET/POST /invoices`, `GET /invoices/:id`, `PATCH /invoices/:id/status` (`invoices.update`, solo VOID — SENT rechazado explícitamente para no ser un bypass del permiso especial), `POST /invoices/:id/send` (`invoices.send`), `POST /invoices/:id/payments` (`invoices.update`). `createInvoice` agrega `PayrollItem` no facturado agrupado por Assignment y marca los items incluidos como facturados en la misma transacción. `flagOverdueInvoicesForTenant` (sweep, scheduler propio de 60 min).
- Frontend: `Invoices.tsx` (listado con filtros + generación real vía drawer), `InvoiceDetail.tsx` (acciones gateadas por permiso y estado, registro de pago, anulación con confirmación).

### 23.4 Verificación (evidencia, no autodeclaración)

- **Backend:** 14 tests nuevos (`billing/billing.test.ts`) — RBAC, rechazo sin PayrollItem facturable (ninguno o PayrollRun todavía DRAFT), generación real con subtotal/total/balance verificados matemáticamente, no doble facturación del mismo PayrollItem (verificado explícitamente con una segunda generación para el mismo período), tenancy, SENT solo alcanzable vía el endpoint dedicado (rechazado por el genérico), PAID manual siempre rechazado, ciclo completo envío→pago parcial→pago total con auto-transición a PAID verificada, pago que excede balance rechazado, pago rechazado antes de SENT, VOID terminal, sweep de OVERDUE que nunca toca DRAFT/PAID. **Regresión completa: 320/320 tests** (306 preexistentes de F0–F5.7 + 14 nuevos).
- **Frontend/Playwright, navegador real, desktop (1440×900) y mobile (iPhone 13, 390×844):** generación real de un Invoice desde un PayrollRun APPROVED sembrado ($350) → detalle con la línea correcta → envío → pago parcial ($100, balance $250 verificado en pantalla) → pago final (balance $0, auto-transición a Paid verificada, botones de acción correctamente ocultos tras cerrarse). Cero errores de consola, cero requests fallidos. Fixture de verificación eliminado sin dejar residuales (candidates=40, workers=10, jobOrders=6, invoices=0 sin cambios frente al estado previo).
- `pnpm typecheck`/`lint` limpios en todo el monorepo (solo los 2 warnings preexistentes, no relacionados).
- F0–F5.7 intactos: ningún test preexistente modificado ni roto.

### 23.5 Commits (uno por paso pequeño, sin regenerar/resetear la base en ningún momento)

`56df64b` (modelo Payment + migración + tenancy) → `0a1437b` (PayrollItem.invoiced) → `56275f2` (permiso invoices + grants) → `c7327ac` (contratos compartidos) → `84b8219` (service + router + scheduler) → `13d1241` (tests backend) → `d9ec9a5` (frontend).

### 23.6 Lo que F5.8 explícitamente no incluye (fuera de alcance de F5, no de esta fase)

Projects (CRUD), Matching por IA, cálculos fiscales (decisión D7, delegado a un proveedor externo en una fase posterior), PDF de invoice (§7.2 de la arquitectura original — Billing produce datos reales, no un documento renderizado todavía) — nada de esto se tocó, tal como se acordó.

---

**Este documento fue de planificación para F5 completo; §16-§23 documentan el resultado real de los ocho bloques implementados (F5.1-F5.8). Con el cierre de F5.8, F5 — Staffing Operations queda completamente implementado y verificado. El resto de §10–§12 no cubierto arriba (Projects, Matching por IA) queda fuera del alcance mandatado para esta fase.**
