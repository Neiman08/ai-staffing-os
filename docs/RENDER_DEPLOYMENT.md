...
**Nota de compatibilidad real (F14, 3 intentos de importación/build)**: el primer intento de importar `render.yaml` en Render falló con "static sites cannot have a region" (`ai-staffing-os-web`/`ai-staffing-os-marketing`) — corregido quitando `region` de ambos servicios estáticos (nunca es un campo válido ahí, un sitio estático se sirve desde la CDN global de Render). El segundo intento falló con "Legacy Postgres plans, including 'starter', are no longer supported for new databases" — corregido cambiando el plan de `ai-staffing-os-db` de `starter` (legacy) a `basic-256mb` (plan flexible vigente, mismo precio ~$7/mes, ver `render.com/docs/postgresql-refresh`). El plan `starter` de `ai-staffing-os-api` **no cambió** — la deprecación fue específica de Postgres, sigue siendo válido para Web Services. El tercer intento (ya con el Blueprint importado) falló durante el **build** de los 3 servicios de Node/pnpm con `EROFS: read-only file system, unlink '/usr/bin/pnpm'` — causado por `corepack enable &&` al inicio de cada `buildCommand`, que intentaba reinstalar un pnpm que la imagen de build de Render ya deja preparado (de solo lectura) según `packageManager` de `package.json`; ejecutarlo de nuevo era redundante y rompía el build. Quitado de los 3 `buildCommand` (ahora empiezan directo con `pnpm install --frozen-lockfile`). De paso se fijó `engines.node: "22.x"` en el `package.json` raíz — antes `>=20`, que Render venía resolviendo a Node 26.5.0 (una major muy posterior a la que este proyecto probó), y se confirmó que `packageManager` (`pnpm@9.15.9`) ya era una versión explícita del gestor de paquetes.
...
| **Build command** | — | `pnpm install --frozen-lockfile && pnpm --filter @ai-staffing-os/db run generate && pnpm --filter @ai-staffing-os/db run migrate:deploy` | `pnpm install --frozen-lockfile && pnpm --filter @ai-staffing-os/web run build` | `pnpm install --frozen-lockfile && pnpm --filter @ai-staffing-os/marketing run build` |
...
| **Node version** | — | `22.x` (de `engines.node` en `package.json` raíz — F14: antes `>=20`, que Render resolvía a la última major disponible en vez de quedarse en una LTS conocida y probada) | `22.x` (mismo `engines.node`, solo para el build) | `22.x` (ídem) |
