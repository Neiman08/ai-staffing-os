...
**Estado:** **implementación parcial real, verificada** — §1 (Website Intelligence), §2 (email discovery: website + Hunter.io) y §3 (email verification: Hunter.io) implementados y verificados con datos reales. §6–§9 (Gmail OAuth, envío controlado, deliverability, inbox) **siguen sin implementar** — el Product Owner pidió explícitamente avanzar solo con la parte de descubrimiento/verificación de email y mantener todo lo de branding/dominio/Gmail/envío configurable hasta decidir el nombre comercial del staffing. Ver "Addendum — implementación real (2026-07-10)" al final.
...
model GmailConnection {
  ...
  emailAddress      String                // cuenta de Workspace conectada, ej. ventas@<dominio-a-decidir>
...
### B5 — Dirección postal comercial y nombre comercial (CAN-SPAM, §13.3) — explícitamente pendiente
- No es una credencial, es un dato real que solo el PO tiene: la dirección postal comercial real y el nombre comercial definitivo a usar en el pie de cada correo. **El PO indicó explícitamente que el nombre del staffing todavía no está decidido** — nada en el código de F4.7 hardcodea una marca ni un dominio; ambos quedan como configuración de `Tenant.settings` (`senderIdentity`, `businessAddress`), sin valor por defecto que sugiera un nombre.
- Bloqueante para cumplir CAN-SPAM §13.3 antes de cualquier envío real (no bloquea Website Intelligence/discovery/verification, que no envían nada — por eso se pudo avanzar con esas tres piezas sin esperar esta decisión).

### B6 — Acceso/confirmación de DNS del dominio de envío (§8)
- Para que la página de Deliverability muestre datos reales, hace falta saber **cuál es el dominio real de envío** (¿el mismo dominio del sitio de la agencia? ¿un subdominio dedicado tipo `mail.<dominio-a-decidir>`, recomendado para no arriesgar la reputación del dominio principal?) — decisión de producto, no solo de código. **Explícitamente pendiente: el nombre comercial/dominio del staffing todavía no está decidido — nada en el código de F4.7 hardcodea un dominio ni una marca, ver addendum.**
- No bloquea el desarrollo de la página de verificación en sí (que solo hace queries DNS de lectura), pero sí bloquea que el resultado sea significativo antes de tener el dominio confirmado.
...
## Addendum — implementación real (2026-07-10)
...
Todo lo que este documento ya marcaba como bloqueado por B3 (Google Cloud OAuth), B5 (marca/dirección postal) y B6 (dominio de envío): `GmailConnection`, `EmailMessage`, envío real, deliverability (SPF/DKIM/DMARC), lectura de inbox, y el Email Intelligence Dashboard de §10 (dashboard agregado — hoy los datos ya están completos y consultables vía `Contacts.tsx`/`CompanyDetail`, pero no hay una vista agregada de "cobertura por proveedor"/"costo por email válido" todavía). Se retoma cuando el PO decida el nombre comercial y resuelva B3/B5/B6.
