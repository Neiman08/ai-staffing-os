...
| `VITE_APP_URL` | No (sin ella, el botón "Login"/`/login` quedan deshabilitados/en loading indefinido — nunca rotos, nunca un link falso) | `src/lib/app-url.ts` (`APP_URL`), consumido por `components/layout/Header.tsx` y `pages/Login.tsx` | F14: reemplaza al mecanismo anterior (derivar la URL del CRM en runtime desde `branding.appDomain`, que a su vez leía `APP_DOMAIN` del backend — atado al dominio propio `app.dreistaff.com`, que hoy no está conectado a ningún deploy real). Con esta variable, "adónde lleva Login" es una decisión 100% del frontend, independiente del backend. **Hoy** (mientras no haya dominio propio conectado): `https://ai-staffing-os-web.onrender.com`. Cuando `app.dreistaff.com` esté listo: cambiar el valor acá y redeployar solo `ai-staffing-os-marketing` — nunca requiere tocar código. |
