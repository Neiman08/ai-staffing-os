...
export default function CandidateDocumentsPage() {
  ...
  const [submittingItem, setSubmittingItem] = useState<CandidateDocumentItem | null>(null);
  ...
  const submitMutation = useMutation({
    mutationFn: (input: { fileName: string; notes: string | null }) =>
      apiFetch(`/portal/candidate/documents/${submittingItem!.id}/submit`, { method: "POST", body: JSON.stringify(input) }),
    onSuccess: () => {
      toast({ title: "Documento enviado", variant: "success" });
      queryClient.invalidateQueries({ queryKey: ["portal-candidate-documents"] });
