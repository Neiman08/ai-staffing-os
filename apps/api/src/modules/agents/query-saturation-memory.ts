 ...
 * mission-executor.ts (`${taxonomyKey}::${searchTerm}`) para que el
 * llamador no tenga que reconstruir nada.
 */
export async function getQuerySaturationMap(tenantId: string, queries: QueryIdentity[]): Promise<Map<string, QuerySaturationResult>> {
  ...
  // Agrupa por (normalizedQuery, state) real -- el mismo searchTerm en
  // dos estados distintos nunca comparte saturación (mercados distintos).
  const uniqueIdentities = new Map<string, QueryIdentity>();
  for (const q of queries) {
    const key = `${normalizeQueryForSaturation(q.searchTerm)}::${q.state ?? ""}`;
  ...
  const rows = await scopedDb.discoveryQueryExecution.findMany({
    ...
    select: { normalizedQuery: true, state: true, rawResultCount: true, acceptedCount: true, duplicateCount: true, rejectedCount: true, executedAt: true },
  });

  const rowsByIdentity = new Map<string, QuerySaturationSample[]>();
  for (const row of rows) {
    const identityKey = `${row.normalizedQuery}::${row.state ?? ""}`;
  ...
  for (const q of queries) {
    const identityKey = `${normalizeQueryForSaturation(q.searchTerm)}::${q.state ?? ""}`;
    const queryKey = `${q.taxonomyKey}::${q.searchTerm}`;
